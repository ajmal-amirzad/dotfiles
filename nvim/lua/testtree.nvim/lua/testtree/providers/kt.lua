  ...
  for child in root:iter_children() do
    if child:type() == "package_header" then
      ...
      for sub in child:iter_children() do
        if sub:type() == "simple_identifier" or sub:type() == "identifier" then
...
local function build_class(node, bufnr, pkg, class_stack)
  ...
  if not name_node or not (name_node:type() == "type_identifier") then
    name_node = node:named_child(1)
  end

  if not name_node or not (name_node:type() == "type_identifier") then
    name_node = node:named_child(2)
  end

  if not name_node or not (name_node:type() == "type_identifier") then
    name_node = node:named_child(3)
  end
...
function M.get_items()
  ...
  if not ok or not parser then
    vim.notify("Kotlin Treesitter parser not available", vim.log.levels.ERROR)
    return {}
  end
